...
- **Disaster recovery (DR)** — **multi-AZ within the primary region is also the standard DR posture**: zone-redundant infrastructure plus tested in-region backups and restore. Do not flag the absence of cross-region DR as a finding by default. **Cross-region or cross-cloud DR is non-standard** — it needs an explicit, stated justification (regulatory mandate, stated RTO/RPO that must survive full region loss, data residency). Unjustified multi-region DR is an over-engineering finding (cost, complexity, and it usually drags synchronous dependencies across regions, violating the co-location rules above).

Two checks keep this honest: if stated tolerance levels genuinely require surviving region loss, multi-AZ alone is a gap — say so and name the cheapest pattern that meets the requirement (backup copies elsewhere → pilot light → warm standby → active-active, in that order). And if no tolerances are stated, note region loss as an accepted residual risk under the standard posture rather than inventing a multi-region requirement.
